...
function TextStep({ block }: { block: TextBlock }) {
  return <section className={'step-text variant-' + block.variant}><span className="step-type">{block.variant === 'definition' ? '定义' : block.variant === 'example' ? '例子' : block.variant === 'summary' ? '小结' : block.variant === 'callout' ? '提示' : '说明'}</span><p>{block.content}</p></section>;
}

function CodeStep({ block }: { block: CodeBlock }) {
  return <section className="step-code"><div className="code-topline"><span>{block.language || 'code'}</span><span>示例</span></div><pre><code>{block.code}</code></pre>{block.explanation && <p>{block.explanation}</p>}</section>;
}
...
function QuizStep({
  ...
  return <section className="step-quiz"><span className="step-type">小测验</span><h2>{block.question}</h2>{block.options?.length ? <div className="quiz-options">{block.options.map((option) => <label key={option.id} className={chosen.includes(option.id) ? 'selected' : ''}><input type={multiple ? 'checkbox' : 'radio'} name={'quiz-' + block.id} checked={chosen.includes(option.id)} onChange={() => choose(option.id)} /><span>{option.text}</span></label>)}</div> : <textarea value={answer} onChange={(event) => setAnswer(event.target.value)} rows={4} placeholder="用自己的话写下理解…" />}{assessment && <div className={'quiz-feedback ' + assessment.result}><strong>{assessment.result === 'correct' ? '回答正确' : assessment.result === 'partial' ? '方向接近' : '再想一步'}</strong><span>{assessment.feedback}</span></div>}<button type="button" className="btn-primary" disabled={busy || submitted || !answer.trim()} onClick={() => onSubmit(block.id, answer)}>{submitted ? '答案已提交' : '提交答案'} <span aria-hidden="true">→</span></button></section>;
}
