...
export function LessonCanvas({
  ...
  return <article className="lesson-canvas" aria-busy={busy}><header className="lesson-canvas-heading"><span className="eyebrow">当前学习步骤</span><h1>{lesson.title}</h1>{lesson.objective && <MarkdownContent content={lesson.objective} />}</header><div className="active-step-area">{activeBlock?.type === 'text' && <TextStep block={activeBlock} />}{activeBlock?.type === 'code' && <CodeStep block={activeBlock} />}{activeBlock?.type === 'diagram' && <DiagramStep block={activeBlock} />}{activeBlock?.type === 'quiz' && <QuizStep block={activeBlock} assessment={assessment} busy={busy} submitted={submitted} onSubmit={onQuizSubmit} />}{!activeBlock && <div className="step-empty"><span className="eyebrow">本轮步骤</span><h2>这一轮步骤已经完成。</h2><p>返回课程路径，查看你的学习进度。</p></div>}</div>{activeBlock && activeBlock.type !== 'quiz' && <button type="button" className="step-advance" disabled={busy || !canAdvance} onClick={onAdvance}>继续 <span aria-hidden="true">→</span></button>}{activeBlock?.type === 'quiz' && <button type="button" className="step-advance" disabled={busy || !submitted || !canAdvance} onClick={onAdvance}>继续 <span aria-hidden="true">→</span></button>}{busy && <div className="canvas-busy" role="status" aria-live="polite"><span className="busy-dot" />正在调整当前步骤…</div>}</article>;
}
